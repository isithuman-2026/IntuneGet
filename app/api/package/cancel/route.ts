...
export async function POST(request: NextRequest) {
  try {
    ...
    if (dismiss && terminalStatuses.includes(typedJob.status)) {
      ...
      // is_auto_update is Supabase-only (no SQLite column); undefined there is fine.
      if ((typedJob as unknown as { is_auto_update?: boolean }).is_auto_update) {
    ...
    // Optimistic lock for active jobs only (prevent races with a concurrent
    // status change); dismissed jobs (completed/failed) are already
    // guaranteed not cancelled/deployed by the checks above.
